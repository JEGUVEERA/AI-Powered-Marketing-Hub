...
export function SentimentAnalysis() {
  ...
  const handleGenerateResponse = () => {
    ...
    setTimeout(() => {
      setCreativeResponse(
        `🌈 *Creative Response*: Imagine a world where "${text}" becomes more than words — it sparks a movement, evokes emotion, and paints a vision. ✨ Let your thoughts shine!`
      )
  ...
  const handleTrace = () => {
    ...
    setTimeout(() => {
      ...
      setAgentTrace(`
🧠 Thought: Should I use tools? Yes
🔧 Action: AnalyzeSentiment
📥 Input: "${text}"
🔍 Observation: ${trace}
✨ Creative Output: Imagine "${text}" as the seed of a story. What blossoms from it?
`)
      setIsTracing(false)
    }, 1000)
  }
  ...
  const emotionIcon = (emotion: string | null) => {
    switch (emotion) {
      case "Joy": return <Smile className="text-yellow-400 animate-bounce" />
      case "Sadness": return <Frown className="text-blue-400 animate-pulse" />
      case "Anger": return <Flame className="text-red-600 animate-ping" />
      case "Fear": return <Zap className="text-purple-500 animate-pulse" />
      case "Love": return <Heart className="text-pink-500 animate-bounce" />
      case "Surprise": return <Info className="text-orange-400 animate-bounce" />
      default: return <Meh className="text-gray-400" />
    }
  }

  return (
    <Card className="w-full max-w-3xl mx-auto mt-10 bg-gradient-to-br from-white to-slate-100 shadow-2xl rounded-3xl border border-gray-200 p-6 transition-all">
      <CardHeader>
        <CardTitle className="text-3xl font-bold text-center flex items-center gap-3 text-purple-600">
      ...
      <CardContent className="space-y-6">
        <Textarea
          ...
          className="min-h-[140px] rounded-xl border border-gray-300 focus:ring-2 focus:ring-purple-300 transition-all"
        />

        <div className="flex gap-3 flex-wrap justify-center">
          <Button onClick={handleAnalyze} disabled={isAnalyzing} className="bg-gradient-to-r from-green-400 to-green-600 text-white hover:scale-105 transition-transform">
            {isAnalyzing ? "Analyzing..." : "Analyze"}
          </Button>
          <Button onClick={handleGenerateResponse} disabled={!sentiment || !emotion || isGenerating} className="bg-gradient-to-r from-indigo-500 to-purple-600 text-white hover:scale-105 transition-transform">
            {isGenerating ? "Generating..." : "Creative Response"}
          </Button>
          <Button variant="secondary" onClick={handleTrace} disabled={!sentiment || isTracing} className="hover:scale-105 transition-transform">
            {isTracing ? "Tracing..." : "Show Agent Trace"}
          </Button>
        </div>
        ...
        {sentiment && (
          <div className="flex items-center justify-center gap-2 text-lg font-medium">
            ...
            <Badge variant="outline" className="text-base bg-white border-gray-300 shadow-sm">Sentiment: {sentiment}</Badge>
          </div>
        )}
        ...
        {emotion && (
          <div className="flex items-center justify-center gap-2 text-lg font-medium">
            {emotionIcon(emotion)}
            <Badge variant="outline" className="text-base bg-white border-gray-300 shadow-sm">Emotion: {emotion}</Badge>
          </div>
        )}

        {creativeResponse && (
          <div className="bg-white p-4 rounded-xl shadow-md border-l-4 border-purple-500 flex items-start gap-3">
            <Bot className="text-purple-500 mt-1" />
            <div className="text-gray-700">{creativeResponse}</div>
          </div>
        )}

        {agentTrace && (
          <pre className="bg-slate-100 p-4 rounded-xl text-sm text-gray-600 whitespace-pre-wrap border-l-4 border-gray-300 mt-2">{agentTrace}</pre>
        )}
      </CardContent>
    </Card>
  )
